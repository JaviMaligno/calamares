...
1. Conjetura del umbral áureo (τ = φ): la región de la sartén se reduce a UNA celda — |S| ≥ 3 a sartén está cerrado (Teoremas DP-p y DPr, `docs/drafts/perfilp.md` y `docs/drafts/rstar.md`) salvo {p ≥ 4, σ₁+M ≤ 1, j ≥ 3} (vía identificada: corona cíclica a nivel de sartén, R ≥ R_corona(O ∪ {m})); quedan además los pequeños extra (convergen a la misma corona), el régimen de pivote sólido ω ≥ 1 con j ≥ 3, y ensamblar el lema universal de reinserción con umbral φ. Del lado anidado hay tres huecos abiertos al nivel áureo — la puntita j = 2 con ω ∈ [φ/2, 1), el gap lemma cuantitativo y los perfiles k ≥ 4 fuera de la rama de reducción — ya DECLARADOS en el paper (párrafo Status y op:assembly(b)); las demás puntitas de anchura solo afectan a fijar el suelo anidado en exactamente T. La micro-celda de j = 3 del par ya está cerrada (Teorema M, `docs/drafts/microcelda.md`).
