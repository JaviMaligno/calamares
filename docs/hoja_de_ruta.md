...
2. **H1** (κ ≥ 1 en la frontera angular del trío; numérico con κ_min = 1.0249). Reformulación atacable: la suma angular F es más sensible al círculo grande, ∂F/∂s₁ ≥ ∂F/∂s₂ cuando s₁ ≥ s₂ (sabor Schur). Una prueba quita el "módulo H1" de todo el bloque de grosor.
