...
**Estado: RESUELTO para la subfamilia rígida (Teorema S, `docs/drafts/suelo_rigido.md`, verificado 10/10).** Toda instancia con R = r₁+r₂, pareja en el agujero y trío infactible cumple ρ > T estricto, para todo w > 0, sin idealización; el ínfimo es T y no se alcanza. La prueba reduce la suficiencia del trío a ψ(u)+ψ(v) ≥ τ vía la identidad sin²(θ/2) = f(a)f(b), y la configuración rígida emerge como extremo por concavidad (no se supone). La versión con holgura R > r₁ + r₂ quedó RESUELTA en `drafts/universal.md` (Corolario U1: la no-empaquetabilidad es decreciente en R y el Teorema S aplica con el disco ficticio r₁ + r₂; además la familia es vacía si R ≥ 2.1547·r₁). El esqueleto de abajo queda como referencia histórica.
...
1. **Contenedores v/u genéricos** — EL hueco. Todo lo demostrado vive en la plantilla canónica (v rígido con un vecino grande, S un par, u el agujero de α). Hace falta: cota inferior universal del mayor hueco de un empaquetamiento en función de la capacidad libre, y la restricción del testigo cuando u es la sartén (empaquetabilidad junto a ocupantes mayores, no capacidad simple). Pista útil ya medida: el umbral bloqueante crece como b(t) + Θ(√δ) fuera de la esquina rígida (rigido.py V7b). **Paso 1 de la Batalla 1 RESUELTO en `drafts/corona.md` (acta en `VEREDICTOS.md`; `code/corona.py` 5/5):** criterio exacto de coronas — LP por orden + certificados de subconjunto (necesidad todo k); **Lema U₄** cerrado en k = 4 (trío top + total zigzag, dos desigualdades); k = 5 exacto combinatorio con el certificado del pentagrama (Teorema C7). OJO: el plan original de este paso era falso tal cual (la suma de arcos consecutivos NO es suficiente para k ≥ 4, y el orden decreciente NO es óptimo — es el zigzag); los pasos 2–4 de la Batalla 1 deben usar el Lema U₄, no aquel enunciado. Sigue abierto el paso 3 (ocupantes interiores → corona), que es el corazón.
...
**Extensiones naturales:** ~~holgura R > r₁ + r₂ en el Teorema S~~ (RESUELTO: Corolario U1 de `drafts/universal.md`); escalera completa y familia de 4 aros del cuadrado; ¿comparten mecanismo la meseta φ de ρ*₃ y el suelo φ del bolsillo (Prop. 2)? (La optimalidad de la rama mixta y de la esquina 13/7 quedó DEMOSTRADA en `drafts/esquina.md`: curva exacta, ω₁ cúbico, α_m séxtica, ínfimo global 13/7 sin módulo del criterio angular.)
