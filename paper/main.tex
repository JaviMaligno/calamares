...
\author{Javier Aguilar\thanks{\texttt{javier.aguilar@sapira.ai}. Draft.
Computational verification scripts for every numerical claim, together with
the full proofs and the adversarial verification reports cited in the
appendix, are available in the accompanying repository.}}
\date{\today}
...
ratio -- as the obstruction. The Tribonacci floor is proved as a theorem,
with no tangency idealization, over the entire rigid subfamily: the rigid
configuration \emph{emerges} from a concavity argument, and the infimum $T$
is exact and not attained. We conjecture that the geometric threshold equals
$T$ and develop the supporting program: with positive relative width
$\omega$ the blocked-exchange infimum is governed by a \emph{deformed
Tribonacci} cubic, with a closed lower-bound curve whose global minimum is
exactly $13/7$ at a rational corner; the purely combinatorial profile
threshold equals
$\max\bigl(1,\min\bigl(2(1-\omega),\max(\varphi,\tfrac{2}{1+2\omega})\bigr)\bigr)$
for every profile size, crossing $T$ at $\omega_T=1/T-1/2$; and for generic
containers a hierarchy of explicit blocking walls (an exact corona
criterion, ``blocking a hole costs its slack'', a double-pocket wall) forces
$\rho$ above metallic-mean floors -- the silver ratio $1+\sqrt2$ and the
golden line $\varphi^2-(\varphi/2)\omega$ appear -- covering, with extra
occupants, widths up to $0.9626$, $0.624$, $0.896$, and all widths,
respectively for one, two, three, and four or more occupants. We complement these results with a phase diagram for the
